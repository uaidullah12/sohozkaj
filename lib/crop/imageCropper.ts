...
export class ImageCropper {
  ...
  constructor(canvasId: string, imageSource: string, initialSettings: Partial<CropSettings> = {}) {
  ...
  public draw(): void {
    ...
    this.ctx.clearRect(this.crop.x, this.crop.y, this.crop.width, this.crop.height);
  ...
  public resizeCrop(dw: number, dh: number, lockAspect: boolean = this.settings.locked): void {
    ...
    if (lockAspect && this.settings.aspectRatio) {
      this.crop.height = this.crop.width / this.settings.aspectRatio;
    }
    this.validateCrop();
    this.draw();
  }
  ...
  public setCropPreset(preset: CropPreset): void {
    ...
    switch (preset.unit) {
      case 'mm':
        widthPx = (preset.width / 25.4) * preset.dpi;
        heightPx = (preset.height / 25.4) * preset.dpi;
        break;
      case 'cm':
        widthPx = (preset.width / 2.54) * preset.dpi;
        heightPx = (preset.height / 2.54) * preset.dpi;
        break;
      case 'inch':
        widthPx = preset.width * preset.dpi;
        heightPx = preset.height * preset.dpi;
        break;
    }
    this.crop = {
  ...
  public getCropInfo(): {
    ...
    const physicalWidth = (this.crop.width / this.settings.dpi) * 25.4;
    ...
    return {
      ...
      output: { width: Math.round(this.crop.width), height: Math.round(this.crop.height) },
    };
  }
