...
app.post('/api/remove-bg', upload.single('image'), async (req, res) => {
  try {
    requireKey(process.env.REMOVEBG_API_KEY, 'remove.bg');
    if (!req.file) return res.status(400).json({ ok: false, error: 'একটি ছবি আপলোড করুন।' });
    const form = new FormData();
    form.append('image_file', new Blob([fs.readFileSync(req.file.path)], { type: req.file.mimetype }), req.file.originalname);
    form.append('size', 'auto');
    const response = await fetch('https://api.remove.bg/v1.0/removebg', {
      method: 'POST', headers: { 'X-Api-Key': process.env.REMOVEBG_API_KEY }, body: form
    });
    if (!response.ok) throw new Error(await response.text());
    const buffer = Buffer.from(await response.arrayBuffer());
    res.json({ ok: true, image: `data:image/png;base64,${buffer.toString('base64')}` });
  } catch (err) {
    console.error(err);
